...
/**
 * A fully functioning tabletserver that will look for a ZK path to signal when to start dropping ZK
 * connections. When the signal node is seen, the BadZooReaderWriter will be used that will always
 * throw errors anytime ZK is accessed from ServerContext. This is an attempt to simulate a tserver
 * having problems communication with ZK but manager doesn't.
 *
...
class BadZooReaderWriter extends ZooReaderWriter {
  ...
  @Override
  public boolean putPersistentData(String zPath, byte[] data, ZooUtil.NodeExistsPolicy policy)
  ...
  public boolean putPrivatePersistentData(String zPath, byte[] data,
      ZooUtil.NodeExistsPolicy policy) throws KeeperException, InterruptedException {
    simulateError();
  ...
  @Override
  public void recursiveDelete(String zPath, ZooUtil.NodeMissingPolicy policy)
      throws KeeperException, InterruptedException {
    simulateError();
